function Recovery_summary(alpha_lc, ptype, gs, use_fig, plot_ttl,plot_xlabel,plot_ylabel,ylabel_str, row_str)
    ...
    for lc=[1 3:13] % too few point for EBF, not include it at for this study
        if isempty(alpha_lc)
            return;
        end
    ...
    for ec=1:2
        ...
        ec_mn=nanmin(alpha_lc(pstm_rng,ec_grp{ec}),[],2); % 1:13, not including normal month
        ec_mean=nanmean(alpha_lc(pstm_rng,ec_grp{ec}),2);
        ec_mx=nanmax(alpha_lc(pstm_rng,ec_grp{ec}),[],2);
    ...
    for lc=valid_ec
        ...
        line_h(lc)=scatter(pstm_rng,alpha_lc(pstm_rng,lc),40,'MarkerFaceColor', eco_color(lc,:),'MarkerEdgeColor', [0 0 0]/255);
    ...
    if plot_ylabel
        textH=text(-2.5, -12, row_str,'Fontsize',10, 'Rotation', 90, 'FontWeight', 'bold');
