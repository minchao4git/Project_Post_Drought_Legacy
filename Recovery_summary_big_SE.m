function Recovery_summary_big_SE(alpha_lc,mdinfo_lc, ptype,subgs)
    ...
    for gs=1:2
        for gsl=gsl_rng
            ...
            if (subgs==1 && gsl==1) || (subgs==2 && gsl==2)
                plot_ttl=1;
            end
            ...
            mtmp=mdinfo_lc{gs, clim,subgs};

            if subgs==2 && gsl==2
                dtmp(2,6,9)=nan;
            end
            a=squeeze(dtmp(gsl,:,:,1)); 
            b=squeeze(mtmp(gsl,1:14,:,1)); % for mdinfo_lc, not including the 12 normal months
            
            Recovery_summary_SE(a,b, ptype, gs,use_fig, plot_ttl, plot_xlabel, plot_ylabel, '\\alpha_l_e_g (%%)', row_str);
